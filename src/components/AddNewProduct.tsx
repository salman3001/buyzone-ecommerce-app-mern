...
const formSchema = yup.object().shape({
    productName: yup.string().min(2, 'minimum two charectors required').max(20, "maximum 20 charectors allowed").required('This field is required'),
    ...
    return (
        <Container component={Paper} maxWidth="xs" elevation={3} sx={{
            ...
            >
                ...
                />
                <Button component='label' color={formik.touched.files && Boolean(formik.errors.files) ? 'error' : 'primary'} variant='contained'>
                    Upload Images
                    <input
                        hidden={true}
                        name='files'
